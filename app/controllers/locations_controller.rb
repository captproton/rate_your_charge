class LocationsController < ApplicationController
  ...
  def index
    if params[:city].present?
      ...
      @location_pins = [{ lat: @current_latitude, lng: @current_longitude, label: 'San Francisco',
                        tooltip: '5 stars' }]
    end
  end

  def autocomplete
    @locations = Location.where('lower(city) LIKE ? OR lower(state) LIKE ?', "%#{params[:q].downcase}%",
                                "%#{params[:q].downcase}%")
